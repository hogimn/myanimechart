...
export const getPreviousSeason = () => {
    ...
    const currentIndex = seasons.findIndex(season => season.season === currentSeason);
    const previousIndex = (currentIndex === 0) ? seasons.length - 1 : currentIndex - 1;
...
export const getCurrentSeasonYear = () => {
    ...
    return currentSeason?.months[0] <= currentMonth ? currentYear : currentYear - 1;
};

export const getPreviousSeasonYear = () => {
    const previousSeason = getPreviousSeason();
    const currentYear = new Date().getFullYear();
    return seasons.find(season => season.season === previousSeason)?.months[0] < 10 ? currentYear : currentYear - 1;
};

export const getNextSeasonYear = () => {
    const nextSeason = getNextSeason();
    const currentYear = new Date().getFullYear();
    return seasons.find(season => season.season === nextSeason)?.months[0] <= 3 ? currentYear + 1 : currentYear;
};
